...
func (g *Generator) generateCreate(resource *ast.ResourceNode) {
	...
	g.indent++
	g.writeLine("return fmt.Errorf(\"validation failed: %s\", err)", "%w")
	g.indent--
	g.writeLine("}")
	g.writeLine("")
	...
	if needsReturningID {
		...
		g.indent++
		g.writeLine("return fmt.Errorf(\"failed to create %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name))
		g.indent--
		g.writeLine("}")
	} else {
...
func (g *Generator) generateFindByID(resource *ast.ResourceNode) {
	...
	g.writeLine("if err != nil {")
	g.indent++
	g.writeLine("return nil, fmt.Errorf(\"failed to find %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name))
	g.indent--
	g.writeLine("}")
	g.writeLine("")
...
func (g *Generator) generateUpdate(resource *ast.ResourceNode) {
	...
	g.writeLine("if err != nil {")
	g.indent++
	g.writeLine("return fmt.Errorf(\"failed to update %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name))
	g.indent--
	g.writeLine("}")
	g.writeLine("")
...
func (g *Generator) generateDelete(resource *ast.ResourceNode) {
	...
	g.writeLine("if err != nil {")
	g.indent++
	g.writeLine("return fmt.Errorf(\"failed to delete %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name))
	g.indent--
	g.writeLine("}")
	g.writeLine("")
...
func (g *Generator) generateFindAll(resource *ast.ResourceNode) {
	...
	g.writeLine("if err != nil {")
	g.indent++
	g.writeLine("return nil, fmt.Errorf(\"failed to query %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name)+"s")
	g.indent--
	g.writeLine("}")
	...
	g.indent++
	g.writeLine("return nil, fmt.Errorf(\"failed to scan %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name))
	g.indent--
	g.writeLine("}")
	...
	g.indent++
	g.writeLine("return nil, fmt.Errorf(\"error iterating %%s: %%%%w\", \"%s\", err)", strings.ToLower(resource.Name)+"s")
	g.indent--
	g.writeLine("}")
	g.writeLine("")
